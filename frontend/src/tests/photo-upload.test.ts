...
describe('PhotoUpload', () => {
	...
	it('renders a gallery file input for photo selection', () => {
	...
	it('gallery input accepts all image files', () => {
		render(PhotoUpload, { props: { onupload } });
	...
	it('renders a camera input with capture="environment" for direct photo capture', () => {
		render(PhotoUpload, { props: { onupload } });

		const input = screen.getByLabelText(/take photo/i) as HTMLInputElement;
		expect(input).toBeInTheDocument();
		expect(input.type).toBe('file');
		expect(input.accept).toContain('image/');
	...
	it('calls onupload with selected file (gallery)', async () => {
		render(PhotoUpload, { props: { onupload } });
	...
	it('calls onupload with captured file (camera)', async () => {
		render(PhotoUpload, { props: { onupload } });
	...
	it('supports multiple file selection on gallery input when multiple prop is true', () => {
	...
	it('camera input never allows multiple selection', () => {
		render(PhotoUpload, { props: { onupload, multiple: true } });
	...
	it('calls onupload for each file when multiple files are selected via gallery', async () => {
		render(PhotoUpload, { props: { onupload, multiple: true } });
	...
	it('disables both inputs when currentCount reaches maxPhotos', () => {
		render(PhotoUpload, { props: { onupload, currentCount: 4, maxPhotos: 4 } });

		const gallery = screen.getByLabelText(/choose photo/i) as HTMLInputElement;
	...
	it('limits onupload calls when gallery files exceed remaining slots', async () => {
