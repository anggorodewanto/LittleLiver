...
## Lab Result AI Extraction

- [x] **Phase 55: Lab extraction backend — Claude Vision API integration**
  **Depends on:** Phase 20 (lab results CRUD), Phase 23 (photo upload to R2)
  **What to build:** New endpoint `POST /api/babies/:id/labs/extract`. Accepts an array of R2 photo keys (max 10), fetches image bytes from R2, sends all images in a single request to the Claude Vision API with a structured extraction prompt, and returns extracted lab results as JSON. The prompt maps to known test names (total_bilirubin, ALT, AST, GGT, etc.) but accepts any test found. Multi-image support handles multi-page lab reports — all images are sent as parts of one report. Deduplicates results across pages (same test_name → keep last occurrence). Returns confidence levels (high/medium/low) per result. **Duplicate detection:** for each extracted result, query existing `lab_results` for same baby where test_name matches (case-insensitive) and value matches within a ±3 day window of the report date (or today). Return any match as `existing_match` on the extracted item so the frontend can warn the user. Rate-limited to 10 requests/hour/user. Does NOT write to the database — purely a read-only suggestion endpoint. Requires `ANTHROPIC_API_KEY` env var.
  **TDD approach:** (1) Test that the handler rejects requests with >10 photo keys (400). (2) Test that invalid/missing R2 keys return 400. (3) Test the extraction service with a **mocked** Claude API client: provide a canned API response and verify the service correctly parses it into the expected response schema (test_name, value, unit, normal_range, confidence). (4) Test deduplication: mock response with duplicate test_name across "pages" → verify only one result per test_name. (5) Test rate limiting: 11th request within an hour returns 429. (6) Test error handling: mock a Claude API failure → verify 502 response. (7) **Test duplicate detection:** seed the DB with an existing lab result (e.g., ALT=45 from 2 days ago), mock an extraction that returns ALT=45 → verify the response includes `existing_match` with the seeded entry's ID and timestamp. (8) Test no false duplicate: seed ALT=45 from 10 days ago (outside ±3 day window) → verify `existing_match` is null. (9) Integration test: mock both R2 (return test image bytes) and Claude API (return canned extraction) → verify full handler flow from request to structured response including duplicate detection.
  **Proof of progress:** All unit and integration tests pass. Handler returns correctly structured extraction results with duplicate detection from mocked Claude API responses.

- [x] **Phase 56: Lab extraction frontend — upload and review flow**
  **Depends on:** Phase 55, Phase 47 (lab form UI)
  **What to build:** Add an "Import from photo" button to the lab logging UI. Flow: (1) User taps button → camera/file picker opens (reuse existing upload infra), allowing **multiple image selection** for multi-page reports. (2) Images upload via existing `POST /api/babies/:id/upload`. (3) Client sends all R2 keys to `POST /api/babies/:id/labs/extract`. (4) Loading state shown during extraction (~3-5s). (5) Review screen displays extracted results in an editable form — each row shows test_name, value, unit, normal_range with confidence indicator (highlight medium/low confidence in yellow/red). (6) **Duplicate detection UI:** rows with `existing_match` show an "Already logged" warning badge and are **unchecked by default** (excluded from save). User can override by checking the row. (7) User can edit any field, delete rows, or add missing results. (8) On confirm, client submits only checked/reviewed results via existing `POST /api/babies/:id/labs` batch endpoint. (9) On cancel, discard extraction and return to lab form.
  **TDD approach:** (1) Component test: render the review screen with mock extracted data → verify all fields are displayed and editable. (2) Test confidence highlighting: medium confidence → yellow indicator, low → red. (3) Test delete row: click remove on a row → verify it's excluded from the confirm payload. (4) Test edit field: change a value → verify the confirm payload reflects the edit. (5) Test confirm flow: mock the labs batch endpoint → verify it's called with the reviewed data. (6) Test cancel flow: verify extraction is discarded and UI returns to the lab form. (7) Test empty extraction: mock an empty `extracted` array → verify a helpful message is shown ("No lab results found in this image"). (8) Test multi-image upload: verify multiple files can be selected and all R2 keys are sent to the extract endpoint. (9) Test loading state: verify spinner/loading indicator appears during extraction. (10) **Test duplicate warning:** render review screen with an item that has `existing_match` → verify "Already logged" badge is shown and row is unchecked by default. (11) **Test duplicate override:** check a duplicate row → verify it's included in the confirm payload. (12) **Test mixed results:** render with 3 items (1 duplicate, 2 new) → verify only the 2 new items are checked by default.
  **Proof of progress:** All component tests pass. User can photograph a lab report, review extracted results, edit if needed, and save — completing the full flow.

- [x] **Phase 57: Lab extraction integration test — end-to-end flow**
  **Depends on:** Phase 55, Phase 56
  **What to build:** End-to-end integration test covering the full extraction pipeline: upload images → extract → review → save. Uses mocked Claude API (not real API calls) but real R2 interactions (or mocked R2 with real image bytes). Verifies that saved lab results match what was confirmed in the review step.
  **TDD approach:** (1) E2E test with Playwright: upload a test lab report image → wait for extraction → verify review screen shows expected values → edit one value → confirm → verify lab results appear in the labs list with correct values. (2) Backend integration test: POST to extract endpoint with pre-uploaded test images → verify response schema. (3) Verify rate limiting persists across the flow (extraction counts toward the per-user limit). (4) Test error recovery: simulate Claude API timeout during extraction → verify user sees error message and can retry.
  **Proof of progress:** E2E test passes covering upload → extract → review → save. Error recovery test passes.
