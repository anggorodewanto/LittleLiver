...
func TestGetUpcomingMeds_NoScheduleOrTimezone(t *testing.T) {
	...
	_, err = CreateMedication(db, baby.ID, user.ID, "TestMed", "10mg", "once_daily", nil, nil, nil, nil)
	if err != nil {
...
func TestCreateMedLog_AllFieldsPopulated(t *testing.T) {
	...
	tz := "UTC"
	sched := `["08:00"]`
	med, err := CreateMedication(db, baby.ID, user.ID, "TestMed", "10mg", "once_daily", &sched, &tz, nil, nil)
	if err != nil {
		t.Fatalf("CreateMedication: %v", err)
	}
...
func TestGetActiveAlerts_MissedMedication_InvalidTimezone(t *testing.T) {
	...
	_, err = CreateMedication(db, baby.ID, user.ID, "TestMed", "10mg", "once_daily", &sched, &invalidTz, nil, nil)
	if err != nil {
		t.Fatalf("CreateMedication: %v", err)
	}
...
func TestGetActiveAlerts_MissedMedication_InvalidScheduleJSON(t *testing.T) {
	...
	_, err = CreateMedication(db, baby.ID, user.ID, "TestMed", "10mg", "once_daily", &badSched, &tz, nil, nil)
	if err != nil {
		t.Fatalf("CreateMedication: %v", err)
	}
...
func TestCreateMedication_WithScheduleAndTimezone(t *testing.T) {
	...
	tz := "America/New_York"
	sched := `["08:00","20:00"]`
	med, err := CreateMedication(db, baby.ID, user.ID, "Ursodiol", "50mg", "twice_daily", &sched, &tz, nil, nil)
	if err != nil {
		t.Fatalf("CreateMedication: %v", err)
	}
...
func TestCreateMedication_WithoutScheduleOrTimezone(t *testing.T) {
	...
	med, err := CreateMedication(db, baby.ID, user.ID, "VitD", "400IU", "once_daily", nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("CreateMedication: %v", err)
	}
...
func TestCreateMedication_ClosedDB(t *testing.T) {
	...
	db := setupTestDB(t)
	db.Close()
	_, err := CreateMedication(db, "b1", "u1", "Med", "10mg", "once_daily", nil, nil, nil, nil)
	if err == nil {
		t.Fatal("expected error for closed DB")
	}
}
