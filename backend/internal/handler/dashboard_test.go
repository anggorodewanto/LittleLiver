...
func TestDashboardHandler_UpcomingMeds_ExcludesDeactivated(t *testing.T) {
	...
	store.CreateMedication(db, baby.ID, user.ID, "Active Med", "10mg", "once_daily", &sched, &tz, nil, nil)
	med2, _ := store.CreateMedication(db, baby.ID, user.ID, "Inactive Med", "5mg", "once_daily", &sched, &tz, nil, nil)
	inactive := false
	store.UpdateMedication(db, baby.ID, med2.ID, user.ID, "Inactive Med", "5mg", "once_daily", &sched, &tz, &inactive, nil, nil)
