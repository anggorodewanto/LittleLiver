...
func TestCreateMedLog_BabyMismatchRejected(t *testing.T) {
	...
	med, err := store.CreateMedication(db, baby1.ID, user.ID, "Ursodiol", "50mg", "twice_daily", nil, nil)
	if err != nil {
		t.Fatalf("CreateMedication failed: %v", err)
	}
...
func TestCreateMedLog_AdHocDose(t *testing.T) {
	...
	med, err := store.CreateMedication(db, baby.ID, user.ID, "Tylenol", "5ml", "as_needed", nil, nil)
	if err != nil {
		t.Fatalf("CreateMedication failed: %v", err)
	}
...
func TestListMedLogs_FilterByMedicationID(t *testing.T) {
	...
	med1, err := store.CreateMedication(db, baby.ID, user.ID, "Ursodiol", "50mg", "twice_daily", nil, nil)
	if err != nil {
		t.Fatalf("CreateMedication failed: %v", err)
	}
	med2, err := store.CreateMedication(db, baby.ID, user.ID, "Vitamin D", "400IU", "once_daily", nil, nil)
	if err != nil {
		t.Fatalf("CreateMedication failed: %v", err)
	}
